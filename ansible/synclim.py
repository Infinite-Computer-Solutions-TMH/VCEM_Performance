...
# def load_config(file_path="configurations.yaml"):
#     with open(file_path, "r") as file:
#         return yaml.safe_load(file)

# def generate_device_ids(device_count, prefix="bot"):
#     ranges = [
#         (1, 1 + device_count),
#         (25000, 25000 + device_count),
#         (50000, 50000 + device_count),
#     ]
#     device_ids = []
#     for start, end in ranges:
#         device_ids.extend([f"{prefix}{num:06d}" for num in range(start, end )])
#     return device_ids
...
#     record_metrics(action_type.lower(), {
#         "latency": latency,
#         "first_byte_time": first_byte_time,
#         "response_time": response_time,
...
#     headers = {**api_config["gpv"]["headers"], "Authorization": f"Bearer {auth_token}"}
#     payload = api_config["gpv"]["payload"]
#     perform_api_request(url, headers, payload, device, "GPV", metrics)
...
#     return auth_token, api_call_counter

# def set_parameters_values(device, auth_token, api_config, metrics, api_call_counter):
#     url = api_config["spv"]["url"].format(device_id=device)
#     headers = {**api_config["spv"]["headers"], "Authorization": f"Bearer {auth_token}"}
#     payload = api_config["spv"]["payload"]
#     perform_api_request(url, headers, payload, device, "SPV", metrics)
...
#         def process_device(device):
#             nonlocal auth_token, api_call_counter, device_counter
#             auth_token, api_call_counter = get_parameters_values(device, auth_token, api_config, metrics, api_call_counter)
#             if not auth_token:
#                 return
#             auth_token, api_call_counter = set_parameters_values(device, auth_token, api_config, metrics, api_call_counter)
#             if not auth_token:
#                 return
#             device_counter += 1
...
#     calculate_averages(metrics)
#     console.print(f"[bold cyan]Total Devices Processed: {device_counter}[/bold cyan]")
#     record_metrics("device_count", {"total_devices": device_counter})
